...
func (s *trafficScheduler) reservePadding(now time.Time, worstWireBytes int) bool {
	if worstWireBytes <= 0 || worstWireBytes > int(paddingBurstBytes) {
		return false
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if s.credit < cost || s.ceiling < cost {
		return false
	}
	s.credit -= cost
	s.ceiling -= cost
...
func (s *trafficScheduler) beginLatest(key semanticChannel, sequence uint64) (*latestPending, error) {
	...
	defer s.mu.Unlock()
	if s.latest == nil {
		s.latest = make(map[semanticChannel]*latestPending)
	}
	slot := s.latest[key]
	if slot == nil {
		if len(s.latest) >= maxLatestPendingChannels {
			return nil, ErrSemanticQueueFull
		}
		slot = &latestPending{}
		s.latest[key] = slot
	}
...
func (c *Conn) TryWritePadding(n int) (bool, error) {
	if n < 0 {
		return false, errors.New("transport: negative padding size")
	}
	if c.closed.Load() {
		return false, net.ErrClosed
	}
	if c.pendingSend.Load() != 0 || !c.sendMu.TryLock() {
		return false, nil
	}
	...
	now := time.Now()
	if c.pendingSend.Load() != 0 || now.Sub(time.Unix(0, c.lastUsefulSend.Load())) < paddingQuietWindow {
		return false, nil
	}
	...
	if !c.traffic.reservePadding(now, cost) {
		return false, nil
	}
	if err := c.writeFrameLocked(framePadding, make([]byte, n)); err != nil {
		return false, err
	}
	return true, nil
}
...
func (c *Conn) writeSemanticMessage(p []byte, obj *CITPObject) error {
	if len(p) > maxPayload {
		return errors.New("citp: object exceeds transport payload limit")
	}
	if obj.Mode < ModeReliableOrdered || obj.Mode > ModePartialReliable {
		return errors.New("citp: invalid delivery mode")
	}
	...
	if slot != nil && !c.traffic.latestCurrent(slot, obj.MonotonicSeq) {
		return ErrObjectSuperseded
	}
