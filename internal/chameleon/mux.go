		...
		case smOpenOK, smOpenErr, smResolveOK, smResolveErr:
			if s := m.get(sid); s != nil && s.openDone != nil {
				...
				if cmd == smOpenErr || cmd == smResolveErr {
					...
					if cmd == smResolveErr && string(payload) == errResolveViaUpstream.Error() {
						e = errResolveViaUpstream
					}
				}
...
func (m *Mux) Open(hostport string) (*Stream, error) {
	...
	if ip := net.ParseIP(strings.Trim(host, "[]")); ip == nil {
		...
		if ro == nil || time.Now().Unix() >= ro.Expiry-30 {
			...
			if resolveErr != nil {
				if !errors.Is(resolveErr, errResolveViaUpstream) {
					return nil, fmt.Errorf("DNS binding: %w", resolveErr)
				}
...
func (m *Mux) SendCITPObject(obj *CITPObject) error {
	if obj == nil {
		return errors.New("citp: nil object")
	}
	if !m.Alive() {
		return m.Err()
	}
	...
	if err := m.conn.writeSemanticMessage(frame, obj); err != nil {
		return err
	}
