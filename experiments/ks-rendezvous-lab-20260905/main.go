...
func main() {
	schedule := flag.String("schedule", "schedule.json", "offline rendezvous configuration")
	at := flag.Int64("at-unix", -1, "planner time; -1 uses local clock, no external clock request")
	mode := flag.String("mode", "probe", "init|server|probe|plan")
	...
	if *mode == "plan" {
		...
		plan, e = planFile(*schedule, now)
		if e == nil {
			e = json.NewEncoder(os.Stdout).Encode(plan)
		}
	} else if *mode == "init" {
		e = fixture(*dir)
	} else {
