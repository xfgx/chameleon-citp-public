...
for n in ['go.mod','go.sum','bin/wintun.dll','scripts/build-ks-windows.sh','internal/chaossync/testdata/ks_r1_fields.json','docs/KS-RESEARCH.md']:
 q=dst/n;q.parent.mkdir(parents=True,exist_ok=True);shutil.copy2(pathlib.Path('/root/backups/ks-integration-20260908-01/staging/wintun.dll') if n=='bin/wintun.dll' else src/n,q)
