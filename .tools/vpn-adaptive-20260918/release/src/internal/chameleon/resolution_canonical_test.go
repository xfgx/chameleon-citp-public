...
func TestResolutionCanonicalRejectsBoundaryCollision(t *testing.T) {
	...
	if !bytes.Equal(legacyResolutionData(a), legacyResolutionData(b)) {
		t.Fatal("fixture is not a legacy boundary collision")
	}
	a.Sign(seed)
	b.Sig = append([]byte(nil), a.Sig...)
	if b.Verify(seed) == nil {
		t.Fatal("altered address boundaries accepted")
	}
	b.Sign(seed)
	if bytes.Equal(a.Sig, b.Sig) {
		t.Fatal("canonical signatures collide")
	}
	decoded, err := DecodeResolutionObject(a.Encode())
	if err != nil || decoded.Verify(seed) != nil {
...
func TestResolutionCanonicalNoLegacySignatureFallback(t *testing.T) {
	...
	if ro.Verify(seed) == nil {
		t.Fatal("legacy HMAC silently accepted")
	}
}
...
func TestResolutionIssuanceRejectsClientMintedObject(t *testing.T) {
	...
	ro.Sign(seed)
	if ro.Verify(seed) != nil {
		t.Fatal("fixture HMAC invalid")
	}
	var issued resolutionIssuance
	if issued.verify(ro, now) == nil {
		t.Fatal("shared-key HMAC mistaken for server issuance")
	}
	if err := issued.remember(ro, now); err != nil {
		t.Fatal(err)
	}
	if err := issued.verify(ro, now); err != nil {
		t.Fatal(err)
	}
	ro.Addrs[0] = "8.8.8.8"
	ro.Sign(seed)
	if issued.verify(ro, now) == nil {
		t.Fatal("client re-signed modified object accepted")
	}
}
...
func TestResolutionIssuanceBoundedAndExpiry(t *testing.T) {
	...
	for i := range maxIssuedResolutions {
		if err := issued.remember(makeRO(i, now.Add(time.Minute)), now); err != nil {
			t.Fatal(err)
		}
	}
	if issued.remember(makeRO(maxIssuedResolutions, now.Add(time.Minute)), now) == nil {
		t.Fatal("issuance cache limit not enforced")
	}
	future := now.Add(2 * time.Minute)
	if err := issued.remember(makeRO(maxIssuedResolutions, future.Add(time.Minute)), future); err != nil {
		t.Fatal(err)
	}
	if len(issued.objects) != 1 {
		t.Fatal("expired issuances not collected")
	}
}
	...
	for _, mode := range []string{"policy", "bad-signature", "wrong-domain", "open-auth-denied", "explicit-upstream"} {
		t.Run(mode, func(t *testing.T) {
			...
			server.onResolve = func(m *Mux, sid uint32, domain string) {
				if mode == "policy" {
					_ = m.send(sid, smResolveErr, []byte("policy deny: test"))
					return
				}
			...
			if mode == "explicit-upstream" {
				if err != nil || stream == nil || legacyOpens.Load() != 1 {
					t.Fatalf("explicit authenticated delegation failed: %v", err)
				}
...
func TestResolutionMalformedCannotBeSigned(t *testing.T) {
	...
	ro.Sign([]byte("seed"))
	if ro.Encode() != nil || ro.Verify([]byte("seed")) == nil {
		t.Fatal("empty address set accepted")
	}
	var nilRO *ResolutionObject
	if nilRO.Verify([]byte("seed")) == nil {
