...
func (o StrategyObservation) valid() bool {
	...
	if _, err := hex.DecodeString(o.Context.Path); err != nil { return false }
...
func (m *StrategySurvivalModel) Estimate(f Flavor, context StrategyContext, now time.Time) StrategyEstimate {
	...
	if !validStrategyFlavor(f) { return e }
	v := strategyVector(f)
	success := 0.0
	for _, o := range m.Snapshot() {
		if o.Context != context || !o.Learnable() { continue }
		age := now.Sub(time.UnixMilli(o.AtUnixMS))
		if age < 0 || age > 7*24*time.Hour { continue }
		x := strategyVector(o.Flavor)
		distance := 0.0
		for i := range v { distance += (v[i]-x[i])*(v[i]-x[i]) }
		weight := math.Exp(-8*distance) * math.Exp(-math.Ln2*float64(age)/float64(strategyHalfLife))
		if weight < 1e-6 { continue }
		e.Support += weight
		e.Samples++
		if o.Outcome.Reason == "survived" { success += weight }
	}
...
func LoadStrategyHistory(path string) ([]StrategyObservation, error) {
	...
	info, err := f.Stat()
	if err != nil { return nil, err }
	if info.Size() > strategyHistoryMaxBytes { return nil, errors.New("strategy history exceeds size limit") }
	...
	for scanner.Scan() {
		...
		if json.Unmarshal(scanner.Bytes(), &o) == nil { model.Record(o) }
	}
	return model.Snapshot(), scanner.Err()
}

func SaveStrategyHistory(path string, observations []StrategyObservation) error {
	if len(observations) > strategyHistoryLimit { observations = observations[len(observations)-strategyHistoryLimit:] }
	...
	for _, o := range observations {
		if !o.valid() { return fmt.Errorf("invalid strategy observation") }
		if err := encoder.Encode(o); err != nil { return err }
	}
