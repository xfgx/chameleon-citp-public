...
func (s *resolutionIssuance) remember(ro *ResolutionObject, now time.Time) error {
	encoded := ro.Encode()
	if encoded == nil || ro.Expiry <= now.Unix() { return errors.New("resolution issuance: invalid object") }
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.objects == nil { s.objects = make(map[[32]byte]int64) }
	for key, expiry := range s.objects { if expiry <= now.Unix() { delete(s.objects, key) } }
...
func (s *resolutionIssuance) verify(ro *ResolutionObject, now time.Time) error {
	encoded := ro.Encode()
	if encoded == nil { return errors.New("resolution issuance: invalid encoding") }
	key := sha256.Sum256(encoded)
	s.mu.Lock()
	defer s.mu.Unlock()
	expiry, ok := s.objects[key]
	if !ok || expiry <= now.Unix() { return errors.New("resolution object was not issued by this session") }
	return nil
}
