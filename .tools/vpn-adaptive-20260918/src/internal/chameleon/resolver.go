...
func (r *ResolutionObject) Encode() []byte {
	if r == nil || len(r.Sig) != sha256.Size { return nil }
	b := r.unsignedData()
	if b == nil { return nil }
...
func (r *ResolutionObject) unsignedData() []byte {
	...
	for _, addr := range r.Addrs {
		if len(addr) > 255 || net.ParseIP(addr) == nil { return nil }
...
func DecodeResolutionObject(b []byte) (*ResolutionObject, error) {
	...
	if r.unsignedData() == nil { return nil, fmt.Errorf("resolution object has invalid fields") }
	return r, nil
}
