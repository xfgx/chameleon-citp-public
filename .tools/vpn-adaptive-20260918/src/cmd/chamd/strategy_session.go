...
func observeStrategySession(session strategyObservedSession, horizon, interval time.Duration, report func(chameleon.StrategyOutcome)) func() {
	...
	go func() {
		...
		finish := func(reason string) {
			...
			if reason == "survived" && (sent == 0 || received == 0) { reason = "idle" }
		...
		for {
			...
			case <-stop:
				...
				return
			case <-timer.C:
				select { case <-stop: finish("censored"); return; default: }
				if session.Alive() { finish("survived") } else { finish("dropped") }
				return
			case <-ticker.C:
				select { case <-stop: finish("censored"); return; default: }
				if !session.Alive() { finish("dropped"); return }
			}
		}
	}()
...
func (m *Manager) startStrategyObservationLocked(mx *chameleon.Mux, flavor chameleon.Flavor, nodePublicKey string) {
	if m.cbr <= 0 || m.strategyObserver == nil { return }
...
func (a *Autopilot) loadStrategyHistory(dir string) {
	...
	if err != nil {
		if !os.IsNotExist(err) { a.m.logf("autopilot: загрузка наблюдений стратегий: %v", err) }
		return
	}
	for _, observation := range observations { a.survival.Record(observation) }
}
