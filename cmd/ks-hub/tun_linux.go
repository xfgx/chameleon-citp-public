...
// tun_linux.go — TUN для хаба. Копия проверенного cmd/ks-vpn/tun_linux.go:
// сознательно дублируем, а не выносим в общий пакет, чтобы НЕ трогать
// код замороженного клиента v11 и живых узловых служб. Любая правка здесь
// не может уронить уже работающие туннели ПК и телефона.
...
// tunDevice — насосу нужны только Read/Write/Close.
...
func configIface(name, ipCIDR string, mtu int) error {
	...
	return ifreqIoctl(name, socsifflags, func(ifr *[40]byte) {
		binary.LittleEndian.PutUint16(ifr[16:18], iffUp|iffRunning)
	})
}

// openTUN — создать TUN (raw IP без PI-заголовка) и настроить без внешних
// команд: MTU/адрес/поднятие через ioctl. MTU 1300 — как у остальных
// туннелей KS.
func openTUN(name, ipCIDR string, allowV6 bool) (tunDevice, string, error) {
	...
	if err := configIface(in, ipCIDR, 1300); err != nil {
		syscall.Close(fd)
		return nil, "", err
	}
	return f, in, nil
